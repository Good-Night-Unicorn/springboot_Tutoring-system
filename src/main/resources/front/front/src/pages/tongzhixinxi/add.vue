<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="公告名称" prop="gonggaomingcheng">
				<el-input v-model="ruleForm.gonggaomingcheng" 
					placeholder="公告名称" clearable :disabled=" false  ||ro.gonggaomingcheng"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="发布人" prop="faburen">
				<el-input v-model="ruleForm.faburen" 
					placeholder="发布人" clearable :disabled=" false  ||ro.faburen"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="公告内容" prop="gonggaoneirong">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`tongzhixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`tongzhixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
