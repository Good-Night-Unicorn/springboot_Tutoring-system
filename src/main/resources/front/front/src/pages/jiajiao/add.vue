<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="家教资质" v-if="type!='cross' || (type=='cross' && !ro.jiajiaozizhi)" prop="jiajiaozizhi">
				<file-upload
					tip="点击上传家教资质"
					action="file/upload"
					:limit="3"
					...
					@change="jiajiaozizhiUploadChange"
					></file-upload>
			</el-form-item>
			<el-form-item class="add-item" v-else label="家教资质" prop="jiajiaozizhi">
				<img v-if="ruleForm.jiajiaozizhi.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.jiajiaozizhi.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.jiajiaozizhi.split(',')" :src="baseUrl+item">
			</el-form-item>
	...
	export default {
		data() {
			return {
				...
				rules: {
					jiajiaozhanghao: [
						{ required: true, message: '家教账号不能为空', trigger: 'blur' },
					],
					mima: [
					...
					jiajiaozizhi: [
						{ required: true, message: '家教资质不能为空', trigger: 'blur' },
					],
					sfsh: [
					],
					shhf: [
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='jiajiaozizhi'){
							this.ruleForm.jiajiaozizhi = obj[o].split(",")[0];
							this.ro.jiajiaozizhi = true;
							continue;
						}
					}
				}else if(type=='edit'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiajiao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiajiao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			jiajiaozizhiUploadChange(fileUrls) {
				this.ruleForm.jiajiaozizhi = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
