	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/tousujianyi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">投诉时间</div>
						<div class="text "  >{{detail.tousushijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('tousujianyi','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('tousujianyi','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('tousujianyi','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">投诉内容</div>
				</div>
				<div class="desc-text" v-html="detail.tousuneirong"></div>
			</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/tousujianyi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此投诉建议？') .then(_ => {
					this.$http.post('tousujianyi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
