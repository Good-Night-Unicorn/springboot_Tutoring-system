<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="预约时长" prop="yuyueshizhang">
				<el-input v-model.number="ruleForm.yuyueshizhang" 
					placeholder="预约时长" clearable :disabled=" false  ||ro.yuyueshizhang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="预约地址" prop="yuyuedizhi">
				<el-input v-model="ruleForm.yuyuedizhi" 
					placeholder="预约地址" clearable :disabled=" false  ||ro.yuyuedizhi"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiajiaoyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiajiaoyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
