	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiajiaoyuyue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">预约编号</div>
						<div class="text "  >{{detail.yuyuebianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">家教账号</div>
						<div class="text "  >{{detail.jiajiaozhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">家教姓名</div>
						<div class="text "  >{{detail.jiajiaoxingming}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">预约时长</div>
						<div class="text "  >{{detail.yuyueshizhang}}</div>
					</div>
					<div class="item">
						<div class="lable">预约地址</div>
						<div class="text "  >{{detail.yuyuedizhi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiajiaoyuyue','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiajiaoyuyue','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jiajiaoyuyue','学习记录')" 
							@click="onAcross('xuexifankui','是','是','fuwuzhuangtai','已学习','已学习,未学习'.split(',')[0])" type="warning">
							学习记录
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jiajiaoyuyue','投诉建议')" 
							@click="onAcross('tousujianyi','是','是','','')" type="warning">
							投诉建议
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('jiajiaoyuyue','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('jiajiaoyuyue','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`jiajiaoyuyue`);
			...
			payClick(){
				...
				localStorage.setItem('paytable','jiajiaoyuyue')
			...
			backClick(){
				...
				this.$router.push({path: '/index/jiajiaoyuyue', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiajiaoyuyueAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此家教预约？') .then(_ => {
					this.$http.post('jiajiaoyuyue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
