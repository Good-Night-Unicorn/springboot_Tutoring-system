		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.jiajiaoxingming" placeholder="家教姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('jiajiaoxinxi','新增')" type="primary" @click="add('/index/jiajiaoxinxiAdd')">
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							...
							<div class="name">小时价格:{{item.xiaoshijiage}}</div>
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('xuekefenlei/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'xuekefenlei'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.jiajiaoxingming != '') searchWhere.jiajiaoxingming = '%' + this.formSearch.jiajiaoxingming + '%';
				if (this.curFenlei != '全部') searchWhere.xuekefenlei = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiajiaoxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiajiaoxinxiDetail', query: params});
			},
	...
	.list-preview {
		...
		.category-3 {
			...
			.item {
				...
				color: inherit;
				background: url(http://codegen.caihongy.cn/20240918/94f266ddff6440de8b7966bd698e3403.png) no-repeat left top / auto 100%,url(http://codegen.caihongy.cn/20240918/9ec19b189c9446b48f87926d287bb395.png) no-repeat right top / auto 100%,url(http://codegen.caihongy.cn/20240918/2ea43be3fb844fcf85b6fd8e3e4e36d3.png) repeat-x center top / auto 100%;
				display: flex;
				font-size: 16px;
