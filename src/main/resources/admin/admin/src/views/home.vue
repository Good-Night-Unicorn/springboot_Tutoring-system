<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('jiajiaoxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{jiajiaoxinxiCount}}</div>
					<div class="name">家教信息总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('jiajiaoyuyue','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{jiajiaoyuyueCount}}</div>
					<div class="name">家教预约总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('huodongbaoming','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
		...
		<!-- 4 -->
		<div class="type4">
			<div id="jiajiaoxinxiChart1" class="echarts1 animate__animated" v-if="isAuth('jiajiaoxinxi','首页统计')"></div>
			<div id="jiajiaoyuyueChart1" class="echarts2 animate__animated" v-if="isAuth('jiajiaoyuyue','首页统计')"></div>
			<div id="jiajiaoyuyueChart2" class="echarts3 animate__animated" v-if="isAuth('jiajiaoyuyue','首页统计')"></div>
			<div id="huodongbaomingChart1" class="echarts4 animate__animated" v-if="isAuth('huodongbaoming','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getjiajiaoxinxiCount();
		if(this.isAuth('jiajiaoxinxi','首页统计')){
			this.jiajiaoxinxiChat1();
		}
		this.getjiajiaoyuyueCount();
		if(this.isAuth('jiajiaoyuyue','首页统计')){
			this.jiajiaoyuyueChat1();
		}
		if(this.isAuth('jiajiaoyuyue','首页统计')){
			this.jiajiaoyuyueChat2();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'jiajiaoxinxiChart1',css:'animate__'},
				{id:'jiajiaoyuyueChart1',css:'animate__'},
				{id:'jiajiaoyuyueChart2',css:'animate__'},
				{id:'huodongbaomingChart1',css:'animate__'},
			]
		...
		getjiajiaoxinxiCount() {
			this.$http({
				url: `jiajiaoxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jiajiaoxinxiCount = data.data
				}
			})
		},
// 1234 饼
		jiajiaoxinxiChat1() {
			this.$nextTick(()=>{
				...
				var jiajiaoxinxiChart1 = echarts.init(document.getElementById("jiajiaoxinxiChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jiajiaoxinxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.addEventListener('resize', () => {
							jiajiaoxinxiChart1.resize()
						})
					}else{
						this.$message({
							message: data.msg,
		...
		getjiajiaoyuyueCount() {
			this.$http({
				url: `jiajiaoyuyue/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jiajiaoyuyueCount = data.data
				}
			})
		},
// 1234 竖
		jiajiaoyuyueChat1() {
			this.$nextTick(()=>{
				...
				var jiajiaoyuyueChart1 = echarts.init(document.getElementById("jiajiaoyuyueChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jiajiaoyuyueChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.addEventListener('resize', () => {
							jiajiaoyuyueChart1.resize()
						})
					}else{
						this.$message({
							message: data.msg,
		...
		jiajiaoyuyueChat2() {
			this.$nextTick(()=>{
				...
				var jiajiaoyuyueChart2 = echarts.init(document.getElementById("jiajiaoyuyueChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jiajiaoyuyueChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.addEventListener('resize', () => {
							jiajiaoyuyueChart2.resize()
						})
					}else{
						this.$message({
							message: data.msg,
