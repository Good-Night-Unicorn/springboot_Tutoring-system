	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.jiajiaozizhi" label="家教资质" prop="jiajiaozizhi" >
					<file-upload
						tip="点击上传家教资质"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.jiajiaozizhi" label="家教资质" prop="jiajiaozizhi" >
					<img v-if="ruleForm.jiajiaozizhi.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.jiajiaozizhi.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiajiaozizhi.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='jiajiaozizhi'){
							this.ruleForm.jiajiaozizhi = obj[o];
...
var objcross = this.$storage.getObj('crossObj');
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiajiao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiajiaoCrossAddOrUpdateFlag = false;
											this.parent.search();
