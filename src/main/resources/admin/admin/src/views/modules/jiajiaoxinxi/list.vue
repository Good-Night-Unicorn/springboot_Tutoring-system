<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiajiaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jiajiaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('jiajiaoxinxi','学科分类占比')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						学科分类占比
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0 20px 20px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiajiaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiajiaoxinxi','家教预约')" @click="jiajiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								家教预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiajiaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('jiajiaoxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jiajiaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<jiajiaoyuyue-cross-add-or-update v-if="jiajiaoyuyueCrossAddOrUpdateFlag" :parent="this" ref="jiajiaoyuyueCrossaddOrUpdate"></jiajiaoyuyue-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="xuekefenleiChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import jiajiaoyuyueCrossAddOrUpdate from "../jiajiaoyuyue/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				jiajiaoyuyueCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			jiajiaoyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiajiaoyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiajiaoxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiajiaoyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiajiaoyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var xuekefenleiChart1 = echarts.init(document.getElementById("xuekefenleiChart1"),'macarons');
					this.$http({
						url: "jiajiaoxinxi/group/xuekefenlei",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].xuekefenlei);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].xuekefenlei
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '学科分类占比'
							...
							// 使用刚指定的配置项和数据显示图表。
							xuekefenleiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								xuekefenleiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.xuekefenlei!='' && this.searchForm.xuekefenlei!=undefined){
					params['xuekefenlei'] = this.searchForm.xuekefenlei
				}
				if(this.searchForm.jiajiaozhanghao!='' && this.searchForm.jiajiaozhanghao!=undefined){
					params['jiajiaozhanghao'] = this.searchForm.jiajiaozhanghao
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussjiajiaoxinxi',query:{refid:id}});
			},
			// 删除
