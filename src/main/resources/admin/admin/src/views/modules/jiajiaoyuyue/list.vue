<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">家教姓名</label>
						<el-input v-model="searchForm.jiajiaoxingming" placeholder="家教姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiajiaoyuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jiajiaoyuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('jiajiaoyuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiajiaoyuyue','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						批量支付
					</el-button>
					<!-- $predictionAlgorithm -->
					<!-- $predictionChValueColumns -->
					<el-button class="btn18" v-if="isAuth('jiajiaoyuyue','家教预约统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						家教预约统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiajiaoyuyue','日收入')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						日收入
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0 20px 20px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiajiaozhanghao"
						label="家教账号">
						<template slot-scope="scope">
							{{scope.row.jiajiaozhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiajiaoxingming"
						label="家教姓名">
						<template slot-scope="scope">
							{{scope.row.jiajiaoxingming}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="xiaoshijiage"
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('jiajiaoyuyue','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiajiaoyuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiajiaoyuyue','学习记录')" @click="xuexifankuiCrossAddOrUpdateHandler(scope.row,'cross','是','是','fuwuzhuangtai','已学习','已学习,未学习'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								学习记录
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiajiaoyuyue','投诉建议')" @click="tousujianyiCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								投诉建议
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiajiaoyuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jiajiaoyuyue','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<tousujianyi-cross-add-or-update v-if="tousujianyiCrossAddOrUpdateFlag" :parent="this" ref="tousujianyiCrossaddOrUpdate"></tousujianyi-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="jiajiaoxingmingChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import tousujianyiCrossAddOrUpdate from "../tousujianyi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				tousujianyiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			xuexifankuiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','jiajiaoyuyue');
			...
			tousujianyiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tousujianyiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiajiaoyuyue');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tousujianyiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tousujianyiCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				if(row.sfsh!='是'){
				...
				this.$storage.set('paytable','jiajiaoyuyue');
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var jiajiaoxingmingChart1 = echarts.init(document.getElementById("jiajiaoxingmingChart1"),'macarons');
					this.$http({
						url: "jiajiaoyuyue/group/jiajiaoxingming",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].jiajiaoxingming);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].jiajiaoxingming
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '家教预约统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							jiajiaoxingmingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								jiajiaoxingmingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `jiajiaoyuyue/value/yuyueshijian/zongjine/日`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.jiajiaoxingming!='' && this.searchForm.jiajiaoxingming!=undefined){
					params['jiajiaoxingming'] = '%' + this.searchForm.jiajiaoxingming + '%'
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "jiajiaoyuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									if(this.shBatchForm.sfsh!='待审核'){
										for(let x in this.shList){
											await this.$http.get('jiajiao/query',{params: {
												jiajiaozhanghao: this.shList[x].jiajiaozhanghao
											}}).then(async rs=>{
												let param = {
													...
													content: `你提交的“家教预约”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
											...
											await this.$http.get('jiazhang/query',{params: {
												jiazhangzhanghao: this.shList[x].jiazhangzhanghao
											}}).then(async rs=>{
												let param = {
													userid: rs.data.data.id,
													title: '家教预约',
